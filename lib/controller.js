...
var movieDbApi = require('./api/moviedb');
var TmdbAPI = require('./api/tmdb');
...
var _getMovieByGenre = Promise.promisify(movieDbApi.getMovieByGenre.bind(movieDbApi));

var getMovieByGenre = Promise.promisify(TmdbAPI.getMovieByGenre.bind(TmdbAPI));
	...
	processMessage: function (_message, callback) {
		...
		sendMessageToConversation(message)
			...
			.then(function (messageResponse) {
				debug('message response object', messageResponse);
				var responseContext = messageResponse.context;
				var intents = messageResponse.intents.map(function (x) { return x.intent; });
				var entities = messageResponse.entities;
				...
				entities.forEach(item => {
					switch (item.entity) {
						case "genre":
							debug('IT IS A FUCKING MOVIEEEEE');
							getMovieByGenre(item.value, function (data) {
								if (data) {
									var randIdx = Math.floor(Math.random() * data['results'].length);
									console.log("Result: " + data['results'][0]);
									messageResponse['output'].text[0] = 'I found a great ' + item.value + ' movie for you!'
									messageResponse['output'].text.push(data['results'][randIdx].original_title + '.');
									callback(null, messageResponse);
									return;
								}
							});
						...
						case "movies":
							...
							var randIdx = Math.floor(Math.random() * genres.length);
							getMovieByGenre(genres[randIdx], function (data) {
								if (data) {
									var randIdx = Math.floor(Math.random() * data['results'].length);
									console.log("Result: " + data['results'][0]);
									messageResponse['output'].text[0] = 'Great! Here is a nice movie for you:';
									messageResponse['output'].text.push(data['results'][randIdx].original_title + '.');
									callback(null, messageResponse);
									return;
								}
				...
				// 		case "movie":
				// 			debug('IT IS A FUCKING MOVIEEEEE');
				// 			getMovieByGenre('action', function(data) {
				// 				console.log("Result: " + data['results'][0]);
				// 				console.log('Message: ', messageResponse.output.text[0]);
				// 				messageResponse.output.text[0] = data.results[0].original_title;
				// 				console.log('Message: ', messageResponse.output.text[0]);
				// 				callback(null, messageResponse);							
				// 			});
				// 			break;				
